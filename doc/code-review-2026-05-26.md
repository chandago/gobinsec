...
- **#17** — [gobinsec/version_pseudo.go](../gobinsec/version_pseudo.go) : suppression des offsets magiques `27`/`8`. Parsing structurel : `strings.Split(text, "-")`, extraction du timestamp comme suffixe de longueur `PseudoVersionTimestampLength = 14` de l'avant-dernière partie (cette dernière peut être préfixée par `0.` ou `pre.0.` selon la forme pseudo-version), puis parsing des 8 premiers caractères (`YYYYMMDD`). Suppression de la constante désormais inutile `PseudoVersionMinimumLength`. Ajout d'un commentaire de doc sur la struct rappelant les trois formes acceptées, et de deux tests ([gobinsec/version_pseudo_test.go](../gobinsec/version_pseudo_test.go)) : forme pre-release et entrées malformées.
- **#18** — [main.go](../main.go), [gobinsec/binary.go](../gobinsec/binary.go), [gobinsec/dependency.go](../gobinsec/dependency.go) : propagation d'un `context.Context` annulable depuis `main` jusqu'à l'appel HTTP NVD. `main` crée le contexte via `signal.NotifyContext(context.Background(), os.Interrupt, syscall.SIGTERM)` ; le contexte traverse `NewBinary` → `GetDependencies` → `LoadVulnerabilities` (worker) → `Dependency.LoadVulnerabilities` → `fetchVulnerabilities`, où `http.NewRequestWithContext` est désormais utilisé. La boucle des workers vérifie `ctx.Done()` entre dépendances. Les `time.Sleep` (entre appels, et `WaitOnTooManyAttempts = 30s`) sont remplacés par un helper `sleepCtx` qui rend la main dès l'annulation : Ctrl+C interrompt désormais immédiatement même pendant un backoff. Les caches (memcache/file) gardent leur API actuelle, le client memcache sous-jacent n'acceptant pas de contexte.
