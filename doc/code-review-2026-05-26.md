...
- **#12** — [gobinsec/binary.go](../gobinsec/binary.go), [gobinsec/dependency.go](../gobinsec/dependency.go) : suppression de la variable globale `var config Config`. `LoadConfig` retourne désormais `(*Config, error)`, propagé par les constructeurs : `Binary` et `Dependency` portent `Config *Config`, `NewVulnerability` reçoit `*Config` et stocke `Strict` sur la `Vulnerability`, `VulnerabilityMatch.Match` prend un `strict bool` explicite, `BuildCache`/`NewCache` reçoivent `*Config`. La globale `CacheInstance` est conservée (hors périmètre).
...
- **#16** — [gobinsec/vulnerability.go](../gobinsec/vulnerability.go), [gobinsec/binary.go](../gobinsec/binary.go), [README.md](../README.md), [test/report.yml](../test/report.yml), [test/report-config.yml](../test/report-config.yml), [build/report.yml](../build/report.yml), [build/report-config.yml](../build/report-config.yml) : renommage `Matchs` → `Matches` (champ de struct, paramètre `InList`, commentaires) et de la clé YAML `matchs:` → `matches:` dans la sortie de rapport et les fixtures d'intégration. **Changement cassant** : tout script externe parsant la sortie de gobinsec doit remplacer la clé `matchs` par `matches`.
- **#17** — [gobinsec/version_pseudo.go](../gobinsec/version_pseudo.go) : suppression des offsets magiques `27`/`8`. Parsing structurel via `strings.Split(text, "-")` : le timestamp est le suffixe de longueur `PseudoVersionTimestampLength = 14` de l'avant-dernière partie (préfixée éventuellement par `0.` ou `pre.0.`), et la date ses 8 premiers caractères. `PseudoVersionMinimumLength` supprimée. Deux tests ajoutés ([gobinsec/version_pseudo_test.go](../gobinsec/version_pseudo_test.go)) : forme pre-release et entrées malformées.
- **#18** — [main.go](../main.go), [gobinsec/binary.go](../gobinsec/binary.go), [gobinsec/dependency.go](../gobinsec/dependency.go) : propagation d'un `context.Context` annulable depuis `main` jusqu'à l'appel HTTP NVD. `main` crée le contexte via `signal.NotifyContext(..., SIGINT, SIGTERM)`, qui descend jusqu'à `http.NewRequestWithContext`. Les workers vérifient `ctx.Done()` entre dépendances ; les `time.Sleep` (cadence + `WaitOnTooManyAttempts`) passent par un helper `sleepCtx`. Ctrl+C interrompt désormais immédiatement, même pendant un backoff. API cache inchangée (client memcache sans context).
- **#19** — *won't fix* : [go.mod](../go.mod) reste à `go 1.26.3`. Abaisser à 1.23 imposerait de rétrograder `github.com/fatih/color` (v1.19 → v1.18) et `golang.org/x/sys` (v0.45 → ≤ v0.30), qui exigent Go 1.25.0 dans leurs versions récentes. Cette régression de dépendances n'est pas souhaitée ; le projet assume une exigence Go récente.
- **#20** — [gobinsec/vulnerability_test.go](../gobinsec/vulnerability_test.go) : correction des messages d'erreur trompeurs aux lignes 31 et 35. Les deux assertions testent que la vulnérabilité **doit** matcher la version, mais le message disait « should not match ». Remplacé par « should match ».
- **#21** — [main.go](../main.go) : les trois `println(...)` (builtin, sortie sur stderr non garantie) sont remplacés par `fmt.Fprintln(os.Stderr, ...)` / `fmt.Fprintf(os.Stderr, ..., err)`. Les `println(fmt.Sprintf(...))` deviennent un seul `Fprintf`. Sortie d'erreur uniformisée avec le reste du code.
